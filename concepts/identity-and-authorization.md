...
| Authorization data | The OpenFGA engine holds one *IAM store* per organization or account, declared by a [`Store` CR](/reference/resources/iamstore-resource.md). Each IAM store carries the relationship-based authorization data for that account and its provider-consumer relationships. |
